...
from YoutubeApiParsingVideoFast import serviceYoutubeClient
import traceback
...
class parsingYoutubeChannelVideo():
    ...
    def __init__(self):
        ...
        self.serviceYoutubeClient = None
    ...
    def start(self):
        self.getServiceYoutubeClient()
        for channel in self.channelList:
            try:
                self.AddQuotasForYoutubeApi()
                videoList = self.parsingVideoList(channel)
                videoDetailList = self.parsingDetailVideoList(videoList)
                self.saveVideoDetail(videoDetailList)
                self.checkQuotasLimit()
            except:
                pprint(traceback.format_exc())

    def getServiceYoutubeClient(self):
        self.serviceYoutubeClient = serviceYoutubeClient.ServiceYoutubeClient()

    def AddQuotasForYoutubeApi(self):
        self.quotas += 111

    def parsingVideoList(self,channel):
        channelId = channel.get('id')
        parsingChannel = self.serviceYoutubeClient.parsingVideoList(channelId)
    ...
    def parsingDetailVideoList(self,videolist):
        videoDetailList = self.serviceYoutubeClient.parsingDetailVideoList(videolist)
